...
class AnthropicChat:
    ...
    def __init__(self, jsonl_file_path, start_line=1):
    ...
    async def get_response(self, user_message):
        ...
        for attempt in range(max_retries):
            try:
                ...
                return f"<chain_of_thought>{response.content[0].text}"
    ...
    async def process_line(self, line, line_number, total_lines):
        try:
            data = json.loads(line)
            if len(data['messages']) < 3:
                raise ValueError(
                    f"Line {line_number}: Expected at least 3 messages, found {len(data['messages'])}")

            user_message = data['messages'][1]['content']
            assistant_message = data['messages'][2]['content']
            merged_message = f"<raw_transcript>\n{user_message}\n</raw_transcript>\n\n<edited_transcript>\n{assistant_message}\n</edited_transcript>"

            response = await self.get_response(merged_message)
            if response:
                data['messages'].append({
                    "role": "reasoning",
                    "content": response
                })
                logging.info(f"Successfully processed line {line_number}/{total_lines}")
                return json.dumps(data)
            else:
                logging.warning(f"Failed to get response for line {line_number}/{total_lines}")
                return None

        except json.JSONDecodeError as e:
            logging.error(f"JSON decode error on line {line_number}/{total_lines}: {e}")
        except IndexError as e:
            logging.error(f"Index error on line {line_number}/{total_lines}: {e}")
        except KeyError as e:
            logging.error(f"Key error on line {line_number}/{total_lines}: {e}")
        except ValueError as e:
            logging.error(str(e))
        except Exception as e:
            logging.error(f"Unexpected error on line {line_number}/{total_lines}: {e}")
    ...
    async def process_jsonl_file(self):
        ...
        with open(self.jsonl_file_path, 'r') as input_file:
            lines = input_file.readlines()[self.start_line - 1:]

        tasks = []
        for line_number, line in enumerate(lines, self.start_line):
            task = self.process_line(line, line_number, total_lines)
            tasks.append(task)
            
            if len(tasks) == 5 or line_number == len(lines) + self.start_line - 1:
...
if __name__ == "__main__":
    ...
    try:
        chat = AnthropicChat(jsonl_file_path, start_line=476)
        asyncio.run(chat.process_jsonl_file())
    except Exception as e:
