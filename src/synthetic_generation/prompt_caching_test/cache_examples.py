...
- You have been provided with a set of examples to follow. Use the examples to guide your editing process. They are not exhaustive, but they are a good starting point. And they are included above.
"""
...
class ConversationParser:
        ...
        for line in lines:
            try:
                ...
                if processed_conversation:
                    self.examples.append(processed_conversation)
    ...
    def process_conversation(self, conversation):
        ...
        try:
            ...
            output = json.dumps(merged_content)
            input = f"<raw_transcript>{user_message['content']}</raw_transcript>"
    ...
    def write_to_xml(self):
        prompt_root = ET.Element("prompt")
        
        instructions_elem = ET.SubElement(prompt_root, "instructions")
        instructions_elem.text = PROMPT
        
        examples_elem = ET.SubElement(prompt_root, "examples")
        for example in self.examples:
            example_elem = ET.SubElement(examples_elem, "example")
            input_elem = ET.SubElement(example_elem, "input")
            input_elem.text = example["input"]
            output_elem = ET.SubElement(example_elem, "output")
            output_elem.text = example["output"]
        
        tree = ET.ElementTree(prompt_root)
...
if __name__ == "__main__":
    ...
    output_file_path = "/Users/adi/Documents/GitHub/data/tmp/finetuning_file/processed_combined.xml"
