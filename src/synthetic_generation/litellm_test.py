...
class AnthropicChat:
    ...
    def __init__(self, jsonl_file_path, start_line=1, max_workers=4):
        self.client = openai.OpenAI(
                    api_key=os.environ.get('LITELLM_KEY'),  # litellm proxy api key
    ...
    def get_response(self, user_message):
        ...
        for attempt in range(max_retries):
            try:
                ...
                return f"<chain_of_thought>{response.choices[0].message.content}"
...
if __name__ == "__main__":
    ...
    try:
        chat = AnthropicChat(jsonl_file_path, start_line=496, max_workers=4)  # Start from line 300, use 4 workers
